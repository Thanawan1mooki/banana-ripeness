...
# #        ถ้า RAM น้อย ให้ลดค่า batch ลง เช่น 8, 4
# # name: ชื่อของ run การฝึกนี้ ผลลัพธ์จะถูกบันทึกใน runs/detect/your_run_name
# results = model.train(data=data_yaml_path, epochs=70, imgsz=640, batch=8, name='banana_ripeness_detector')
# metrics = model.val()
...
# model = YOLO('runs/detect/banana_ripeness_detector33/weights/last.pt')
...
results = model.train(
    ...
    epochs=80,          
    imgsz=640,
    batch=16,            # ✅ ลดจาก 16 → 8 (CPU ไม่ OOM)
    name='banana_ripeness_detector',
    patience=15,        # ✅ early stop ถ้า val ไม่ดีขึ้น 15 epoch ติดกัน
    optimizer='AdamW',  # ✅ ดีกว่า SGD สำหรับ dataset ขนาดนี้
    hsv_h=0.0,          
    hsv_s=0.0,          
    hsv_v=0.0,          
    flipud=0.0, 
...
print("Weights saved in: runs/detect/banana_ripeness_detector/weights/best.pt") 